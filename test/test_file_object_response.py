# coding: utf-8

"""
    NinjaAPI

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.file_object_response import FileObjectResponse

class TestFileObjectResponse(unittest.TestCase):
    """FileObjectResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> FileObjectResponse:
        """Test FileObjectResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `FileObjectResponse`
        """
        model = FileObjectResponse()
        if include_optional:
            return FileObjectResponse(
                id = 56,
                name = '',
                ingest_label = '',
                metadata = {
                    'key' : ''
                    },
                status = ''
            )
        else:
            return FileObjectResponse(
                id = 56,
                name = '',
                ingest_label = '',
                metadata = {
                    'key' : ''
                    },
                status = '',
        )
        """

    def testFileObjectResponse(self):
        """Test FileObjectResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
